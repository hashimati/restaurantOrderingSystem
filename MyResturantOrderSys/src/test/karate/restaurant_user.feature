...
Background:
    ...
    * def customerRegistrationPath = 'users/register/customer'
    * def deliverRegistrationPath = 'users/register/deliver'
    * def user = {id: 'hashimatiRest', password:'helloworld'}
    * def customer = {id: 'hashimati', password:'helloworld'}
    * def deliver = {id: 'hashimatiDeliver', password:'helloworld'}
...
Scenario: Customer User Registration
Given url rootURL.concat(customerRegistrationPath)
And request customer
When method post
Then status 200
And assert (response == 'Success' || response == 'Failed')

Scenario: DeliveryMan User Registration
Given url rootURL.concat(deliversRegistrationPath)
And request driver
When method post
Then status 200
And assert (response == 'Success' || response == 'Failed')
...
Scenario: Customer Ordering

Given url 'http://localhost:8080/login'
And request {username: 'hashimati', password:'helloworld'}
When method post
Then status 200 
And match response == {username:'hashimati' ,'roles':['customer'],access_token:'#notnull',refresh_token:'#notnull',token_type:"Bearer", expires_in:3600}
And def accessToken = response.access_token
